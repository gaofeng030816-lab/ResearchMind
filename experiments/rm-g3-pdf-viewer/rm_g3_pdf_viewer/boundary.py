"""Pure, separately testable input boundary for the PDF.js experiment."""

from base64 import b64encode
from dataclasses import dataclass
from hashlib import sha256

MAX_EXPERIMENT_PDF_BYTES = 10 * 1024 * 1024
MAX_SAFE_SEQUENCE = 2**53 - 1


@dataclass(frozen=True)
class PageTurn:
    """A validated one-page wheel request inside the isolated experiment."""
...
class ShortcutToggle:
    """A validated request to toggle the experiment-only AI panel."""
...
def make_payload(
    ...
    *,
    ...
    if not 0 < len(pdf_bytes) <= MAX_EXPERIMENT_PDF_BYTES:
        raise ValueError("Experimental PDF exceeds the 10 MiB limit")
...
def validate_shortcut_toggle(
    ...
    *,
    ...
    """Validate the experiment's fixed, non-configurable keyboard shortcut."""
