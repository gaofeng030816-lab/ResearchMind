...
> 当前产品实现为已通过用户内部验收的 `3.0.0rc1` V3 本地候选，不是公开发布。
> 用户于 2026-09-21 明确确认 G7 与最终 V3 通过验收；V3-G0–G7 全部完成。
> 当前实现包括本地资料库、可选 Zotero 只读来源、CCv2/pdf.js 页面划词与工作台、
> 显式证据/可编辑草稿、逐 crop 公式识别和 Python/C/Java/Julia/R 静态代码阅读。
> T5-B1 仍只开放外部本地 Python 当前选择的单范围受控替换，不开放执行。
> 不上传包、不创建公开 Release/tag，也不解除 PyMuPDF 许可或其他权限边界。
> 已验收 `2.0.0rc1` V2 继续作为独立历史回归基线。
...
3.0.0rc1 已完成 V3 内部验收；2.0.0rc1 V2 基线继续独立保留。V3 增加
工作资料库、可选 Zotero 来源、可信页面划词、显式笔记草稿、单公式识别和五语言
静态代码阅读，但仍不包含整页 OCR/PDF→LaTeX、PDF 编辑、Zotero 写入/同步、
代码执行或跨文件语义分析。

## 当前能力（3.0.0rc1 V3 Internal Accepted）

- 在“本地资料库”中点击/拖放上传 PDF 或受支持的代码目录，使用独立数据目录保存
...
- 只读打开一个本地代码文件夹，在 2,000 文件/20 MB/单文件 1 MB 上限内
  索引 UTF-8 Python、C、Java、Julia 和 R 源码；
- Python 使用标准库 AST，C/Java/Julia 使用独立 Tree-sitter grammar，R 使用
  保守词法 adapter；统一定位可用函数、类型、方法或行范围并保留解析来源；
...
2. 通过 PDF 上传框选择/拖放一份 PDF，或通过目录上传框选择一个受支持代码目录；
...
- 经资料库点击导入的 PDF/代码目录复制到 `RESEARCHMIND_DATA_DIR/assets/`；
...
- G5 已进入 `3.0.0rc1` V3 内部验收边界；整页 PDF→LaTeX 与本地 OCR 权重
  仍未采用；
...
- 代码目录点击导入接受符合边界的 .py/.c/.h/.java/.jl/.r UTF-8 文件；
  托管修订只读，改变后的项目必须导入为新修订；
- Streamlit 自动测试不能真实操作浏览器文件选择器；服务端上传边界和 AppTest
  导航已覆盖，点击/拖放仍需真实浏览器人工验收；
- 自动打开页面依赖操作系统存在有效的默认 HTTP 浏览器关联。
- 当前静态代码阅读支持 Python/C/Java/Julia/R；解析失败文件仍可按行选择，
  非 UTF-8 文件只显示诊断；不支持 C++、Jupyter、IDE 插件、跨文件语义分析、
  依赖解析、编译、测试或代码执行。
