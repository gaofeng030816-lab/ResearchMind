...
同步日期：2026-09-04（导航更新，历史证据不变）· 状态：V1.1–V1.3.2、T5-A/T5-B1 与 T6-A–T6-D 已完成；2.0.0rc1 已通过 V2 本地内部验收；V3-G0/G1 已完成，V3-G2 Active，附件读取门禁与人工验收待完成

> 本文保留 V1.x 优化的历史过程。T1–T6 历史状态以
> [V1→V2 过渡要求](../V1%20to%20V2过渡要求.md)为准；当前 V3 阶段和门禁改由
> [V2→V3 过渡要求](../V2%20to%20V3过渡要求.md)管理。
...
2026-08-31 已在独立 `experiments/pdf_formula_latex_spike/` 完成第一轮公式证据；
用户随后明确该能力不属于 V2 验收，现仅作为 V3 需求讨论前的候选 Spike：
数字显示公式区域与宽短嵌入图片候选可用现有 PyMuPDF 定位，复杂真实公式的扁平
文字层不能可靠还原 LaTeX；许可自由 crop 的 dots3-note-prev 多模态探针通过。
该结论仍是 Spike，不属于 `2.0.0rc1` 当前实现，也不阻塞 V2 验收。生产接入所需的
`FormulaRegion`、逐条 crop 外发确认和窄 `FormulaVisionProvider` 合同见
[PDF_FORMULA_LATEX_SPIKE.md](./PDF_FORMULA_LATEX_SPIKE.md)，须单独确认后实施。
