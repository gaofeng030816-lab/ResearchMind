...
版本：2026-09-21 · 当前实现基线：3.0.0rc1 V3 Internal Accepted；2.0.0rc1 V2 baseline preserved · 当前阶段：V3-G0–G7 Completed（用户确认）· 不对外发布
...
| V3-G7 V3 加固与内部验收 | Completed（用户确认） | 数据恢复、安全、性能、完整旅程和人工验收 | 自动门禁、用户验收、3.0.0rc1 内部候选提交 |
...
G7 按以下子门禁顺序执行；前一项通过不自动关闭后一项：

- **G7-A 入口基线与完整性审计（Completed）**：锁定 G6 提交，复跑生产回归、
  依赖一致性、编译、迁移/恢复与关键安全边界。
- **G7-B 安全、隐私与依赖审计（Completed，内部）**：检查密钥卫生、网络/
  外发边界、第三方许可、可复核的依赖漏洞结果和失败语义；PyMuPDF 许可仍阻塞
  任何公开或封闭源发布。
- **G7-C 恢复与性能审计（Completed）**：验证 schema 迁移、备份/恢复、托管
  资产一致性、中断补偿、长文档、大代码项目和 V3 持久化基线。
- **G7-D 浏览器与旅程（Completed，自动证据）**：Edge 覆盖资料库、Zotero
  关闭态、PDF 划词/滚轮/快捷键、公式、五语言代码、草稿与 Obsidian 明确保存；
  本轮 CUA 环境不可用，不伪装成人工视觉结果。
- **G7-E 内部候选关闭（Completed，用户确认）**：wheel、隔离安装、启动、
  联合回归和文档已通过；用户于 2026-09-21 明确确认 G7/V3 内部验收。内部版本
  为 3.0.0rc1；仍不公开发布。

2026-09-21 的自动关闭证据：生产测试为 500 passed / 1 个既有 Windows symlink
环境 skip，加入完整已采纳 G3 实验为 599 passed / 1 skip；临时 pip-audit 对
61 个依赖报告 0 个已知漏洞；规模/持久化/备份恢复基准通过；Edge 153 的 G3/G4/
G5/G6 隔离旅程均通过且 0 外部请求、0 页面错误；1,081,267-byte wheel 完成内容
审计，并在全新 Python 3.12 venv 中非 editable 安装、诊断和本机健康检查通过。
完整记录见 `docs/V3_G7_HARDENING_ACCEPTANCE.md`。用户于 2026-09-21 明确
回复“G7通过验收并提交最终V3”，关闭 G7 和 V3 内部验收门禁。该确认不授权
公开发布、tag、release、PyMuPDF 许可结论或任何未批准的权限扩展。
