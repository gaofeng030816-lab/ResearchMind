...
版本：2026-09-04 · 当前实现基线：2.0.0rc1 V2 Accepted + V3-G1/G2 增量 · 当前阶段：V3-G0/G1 Completed；V3-G2 Active（元数据/来源链接已实现，附件读取门禁与人工验收待确认）· 不对外发布
...
| Zotero 连接 | V3-G2 已实现可选 Local API 元数据/来源链接 | 直接附件导入待本地文件读取门禁；真实 Zotero desktop 人工验收未完成 |
...
| V3-G2 Zotero 只读连接 | Active：部分实现 | 本地 API 探测、浏览/链接已实现；直接附件导入停用 | 回归见 G2 验证记录；文件读取门禁和真实 Zotero 人工验收待确认 |
...
只是模拟字节响应原型，不能证明真实附件导入。直接导入按钮已停用，当前用户可
先点击上传 PDF，再链接 Zotero 来源。最新自动回归和环境 skip 见
[docs/V3_G2_ZOTERO_VALIDATION.md](docs/V3_G2_ZOTERO_VALIDATION.md)。

补充门禁（待用户确认）：只读取用户选定且单独确认的一个本地附件；限定批准的
附件根目录，拒绝 UNC/网络地址、路径逃逸、符号链接/重解析点和非 PDF；不修改
源文件；校验类型、大小和内容后通过 G1 暂存/哈希流程复制。具体实现还需证明
路径检查与读取间的替换风险得到控制。未确认前不得读取 API 返回的本地路径。
...
文件读取门禁、真实 Zotero 浏览/导入/重启/离线降级人工证据关闭前，G2 保持
Active，不启动 G3。
...
发现真实附件接口为本地文件重定向，直接导入已停用并等待补充读取门禁，真实
Zotero 人工验收也尚未关闭。该状态不授权 Web API、写请求、同步或全库缓存。
